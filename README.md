...
| 틱 속도 변경   | 게임 틱 속도가 랜덤하게 변경됩니다. (20, 100, 200, 1000) | 전체               | 20초             | tick_speed_change         |
| 체력 변경      | 플레이어의 체력이 강제로 변경됩니다. (1 ~ 20)            | 1명                | 15초             | player_hp_change          |
...
```mermaid
graph LR
Step1["[ ] 어려움 난이도 추가"] --> A["[ ] 플레이어 룰렛은 반드시 2명이 지정됨."]
Step1 --> B["[ ] 어려움 난이도 전용 이벤트 추가 및 변경"]
Step2["[ ] 플레이어 지정권 추가"] --> A2["[ ] 특정 플레이어에게 1회용으로 이벤트를 지정 가능, 다음 이벤트는 반드시 지정한 플레이어에게 이벤트가 선택됨."]
```
